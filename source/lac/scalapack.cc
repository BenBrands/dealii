...
namespace internal
{
  namespace
  {
    ...
    {
      Assert(matrix.n()==(int)factors.size(),ExcDimensionMismatch(matrix.n(),factors.size()));

      if (grid_mpi_process_is_active)
        for (int i=0; i<matrix.local_n(); ++i)
          {
            const NumberType s = factors[matrix.global_column(i)];

            for (int j=0; j<matrix.local_m(); ++j)
    ...
    void scale_rows(ScaLAPACKMatrix<NumberType>       &matrix,
                    ...
                    const bool                         grid_mpi_process_is_active)
    {
      Assert(matrix.m()==(int)factors.size(),ExcDimensionMismatch(matrix.m(),factors.size()));

      if (grid_mpi_process_is_active)
        for (int i=0; i<matrix.local_m(); ++i)
          {
            const NumberType s = factors[matrix.global_row(i)];

            for (int j=0; j<matrix.local_n(); ++j)
